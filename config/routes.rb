...
# Routes mirror the HTTPS/REST mappings of prEN 18222 (Tables 17, 18, 19).
#
# Identifiers (prEN 18219) are URIs/URLs and therefore contain "/", ":" and ".".
...
Rails.application.routes.draw do
  ...
  scope path: "dpp/v1", defaults: { format: :json }, format: false do
    ...
    scope constraints: { product_id: id } do
      get "dppsByProductId/:product_id",
          to: "api/v1/dpps#by_product_id"                # ReadDPPByProductId
      get "dppsByProductIdAndDate/:product_id",
          to: "api/v1/dpps#by_product_id_and_date"       # ReadDPPVersionByProductIdAndDate
    end

    post "dppsByProductIds",
         to: "api/v1/dpps#ids_by_product_ids"            # ReadDPPIdsByProductIds

    # --- 5  Registry API for Register — Table 18 --------------------------
